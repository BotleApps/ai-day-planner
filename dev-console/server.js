...
const COMMANDS = {
  ...
  'local:ios': {
    ...
    desc: 'cap run ios pointing at http://localhost:3000 — syncs, builds and launches the native iOS app in the Simulator. Start the dev server first; needs macOS + Xcode + CocoaPods.',
    cmd: 'CAP_SERVER_URL=http://localhost:3000 npm run cap:run:ios',
    icon: '',
    interactive: true,
  },
  'local:android': {
    ...
    desc: 'cap run android pointing at http://10.0.2.2:3000 (the emulator alias for your machine) — syncs, builds and launches the native Android app. Start the dev server first; needs Android Studio + JDK 17.',
    cmd: 'CAP_SERVER_URL=http://10.0.2.2:3000 npm run cap:run:android',
    icon: '🤖',
    interactive: true,
  },
